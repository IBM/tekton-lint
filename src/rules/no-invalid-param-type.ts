const checkParameterValues = (resourceName, resourceKind, params, report) => {
    for (const param of params) {
        ...
        if (value != null) {
            ...
            report(
                `${resourceKind} '${resourceName}' defines parameter '${param.name}' with wrong type (only strings and arrays of strings are allowed)`,
                param,
            );
        }
    }
    ...
    for (const triggerBinding of Object.values<any>(tekton.triggerBindings)) {
        ...
        for (const param of triggerBinding.spec.params) {
            if (param.value === undefined)
                report(
                    `TriggerBinding '${triggerBinding.metadata.name}' defines parameter '${param.name}' with missing value`,
                    param,
                );
        }
    }
        ...
        for (const task of Object.values<any>(pipeline.spec.tasks)) {
            ...
            for (const param of Object.values<any>(task.params)) {
                if (typeof param.value == 'undefined') {
                    report(
                        `Task '${task.name}' has a parameter '${param.name}' that doesn't have a value in pipeline '${pipeline.metadata.name}'.`,
                        param,
                        'name',
                    );
                }
