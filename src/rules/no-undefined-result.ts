...
const checkUndefinedResult = (pipeline, tekton, report) => (value, path, parent) => {
    ...
    if (!matchingResult) {
        report(
            `In Pipeline '${pipeline.metadata.name}' the value on path '${pathToString(
                path,
            )}' refers to an undefined output result (as '${value}' - '${name}' is not a result in Task '${task}')`,
            parent,
            path[path.length - 1],
        );
    }
        ...
        for (const task of pipeline.spec.tasks) {
            ...
            for (const param of task.params) {
                ...
                if (taskReference) {
                    ...
                    if (!matchingTask) {
                        report(
                            `Task '${task.name}' refers to task '${taskName}' at value of param '${param.name}' but there is no task with that name in pipeline '${pipeline.metadata.name}'`,
                            param,
                            'value',
                        );
                    }
