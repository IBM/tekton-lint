...
export default (docs, tekton, report) => {
    for (const task of Object.values<any>(tekton.tasks)) {
        ...
        for (const [param, n] of Object.entries(occurences)) {
            if (n) continue;
            report(
                `Task '${task.metadata.name}' defines parameter '${param}', but it's not used anywhere in the spec`,
    ...
    for (const condition of Object.values<any>(tekton.conditions)) {
        ...
        for (const [param, n] of Object.entries(occurences)) {
            if (n) continue;
            report(
                `Condition '${condition.metadata.name}' defines parameter '${param}', but it's not used anywhere in the spec`,
                params.find((p) => p.name === param),
            );
        }
    }
    ...
    for (const template of Object.values<any>(tekton.triggerTemplates)) {
        ...
        for (const [param, n] of Object.entries(occurences)) {
            if (n) continue;
            report(
                `TriggerTemplate '${template.metadata.name}' defines parameter '${param}', but it's not used anywhere in the spec`,
                params.find((p) => p.name === param),
            );
        }
    }
    ...
    for (const pipeline of Object.values<any>(tekton.pipelines)) {
        ...
        for (const [param, n] of Object.entries(occurences)) {
            if (n) continue;
            report(
                `Pipeline '${pipeline.metadata.name}' defines parameter '${param}', but it's not used anywhere in the spec`,
                params.find((p) => p.name === param),
            );
        }
    }
